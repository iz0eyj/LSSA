...
router.get('/layout', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const client = (0, garageClient_1.getGarageClient)();
...
router.get('/info', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (response.data.status) {
        ...
        response.data.statistics = {
            ...
            activeNodes: status.nodes ? status.nodes.filter((n) => n.is_up).length : 0,
            layoutVersion: status.layout_version,
            hasStagedLayout: !!status.staged_layout_version,
            zones: status.nodes ? [...new Set(status.nodes.map((n) => n.zone))].length : 0
        };
    }
    res.json(response);
}));
    ...
    if (result.success && result.data) {
        ...
        const stats = {
            nodes: {
                ...
                active: status.nodes ? status.nodes.filter((n) => n.is_up).length : 0,
                inactive: status.nodes ? status.nodes.filter((n) => !n.is_up).length : 0
            },
            layout: {
            ...
            zones: status.nodes ? [...new Set(status.nodes.map((n) => n.zone))] : [],
...
router.get('/statistics', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (result.success && result.data) {
        ...
        const statistics = {
            ...
            connected_nodes: status.nodes ? status.nodes.filter((n) => n.is_up).length : 0,
            storage_nodes: status.nodes ? status.nodes.filter((n) => n.data_partition !== null).length : 0,
            storage_nodes_ok: status.nodes ? status.nodes.filter((n) => n.is_up && n.data_partition !== null).length : 0,
...
router.get('/advanced-statistics', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const client = (0, garageClient_1.getGarageClient)();
...
router.get('/advanced-health', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const client = (0, garageClient_1.getGarageClient)();
...
router.get('/health', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (result.success && result.data) {
        ...
        const activeNodes = status.nodes ? status.nodes.filter((n) => n.is_up).length : 0;
        const storageNodes = status.nodes ? status.nodes.filter((n) => n.data_partition !== null).length : 0;
        const storageNodesOk = status.nodes ? status.nodes.filter((n) => n.is_up && n.data_partition !== null).length : 0;
