...
import { CreateBucketRequest, UpdateBucketRequest, UploadObjectRequest } from '../types';
...
// Ottieni dettagli di un bucket specifico
router.get('/:bucketId', asyncHandler(async (req, res) => {
...
// Aggiorna un bucket esistente
router.put('/:bucketId', asyncHandler(async (req, res) => {
  const { bucketId } = req.params;
  const updateData: UpdateBucketRequest = req.body;
...
// Elimina un bucket
router.delete('/:bucketId', asyncHandler(async (req, res) => {
  const { bucketId } = req.params;
  const client = getGarageClient();
  
  console.log('🗑️ DELETE BUCKET DEBUG - Tentativo eliminazione bucket:', { bucketId });
  
  // Prima verifica se il bucket esiste e ottieni i suoi dettagli
  ...
  // Verifica se il bucket contiene oggetti
  try {
    console.log('🔍 DELETE BUCKET DEBUG - Verifica contenuto bucket...');
    
    // Prova a usare l'ID del bucket per listare gli oggetti
    ...
    console.log('⚠️ DELETE BUCKET DEBUG - Errore verifica oggetti (procedo comunque):', (objectsError as any).message);
    // Procediamo comunque con l'eliminazione, l'errore potrebbe essere dovuto a permessi S3
  }
  ...
  if (result.success) {
    ...
    // Determina il codice di stato basato sull'errore
    let statusCode = 400;
...
// Lista oggetti in un bucket
router.get('/:bucketName/objects', asyncHandler(async (req, res) => {
...
router.post('/:bucketName/objects', upload.single('file'), asyncHandler(async (req, res) => {
  const { bucketName } = req.params;
  ...
  // Verifica se il bucket esiste e ottieni i suoi dettagli
  const client = getGarageClient();
  console.log('🔍 UPLOAD DEBUG - Verifico esistenza bucket:', bucketName);
  
  const bucketsResult = await client.listBuckets();
  if (bucketsResult.success && bucketsResult.data) {
    const targetBucket = bucketsResult.data.find(b => 
      b.id === bucketName || 
  ...
  const uploadRequest: UploadObjectRequest = {
    key: key,
...
router.get('/:bucketName/objects/:objectKey', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Imposta gli header appropriati per il download
    ...
    // Headers rimossi: metadata non disponibile nel tipo ApiResponse
...
router.delete('/:bucketName/objects/:objectKey', asyncHandler(async (req, res) => {
  const { bucketName, objectKey } = req.params;
...
// Aggiorna configurazione sito web del bucket
router.put('/:bucketId/website', asyncHandler(async (req, res) => {
  const { bucketId } = req.params;
...
// Aggiorna configurazione CORS del bucket
router.put('/:bucketId/cors', asyncHandler(async (req, res) => {
  const { bucketId } = req.params;
...
// Aggiungi alias globale al bucket
router.post('/:bucketId/aliases', asyncHandler(async (req, res) => {
  const { bucketId } = req.params;
...
// Rimuovi alias globale dal bucket
router.delete('/:bucketId/aliases/:alias', asyncHandler(async (req, res) => {
